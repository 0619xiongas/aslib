...
{
	tm* ti = GetLocalTime();
	snprintf(str, TIME_STR_MAX_LEN, "%4d-%02d-%02d %02d:%02d:%02d",
		ti->tm_year + 1900,
		ti->tm_mon + 1,
		ti->tm_mday,
		ti->tm_hour,
...
{
	tm* ti = GetLocalTime();
	snprintf(str, TIME_STR_MAX_LEN, "%4d-%02d-%02d", ti->tm_year + 1900, ti->tm_mon + 1, ti->tm_mday);
}

void asTime::GetLocalTimeStrYMDH(char* str)
{
	tm* ti = GetLocalTime();
	snprintf(str, TIME_STR_MAX_LEN, "%4d-%02d-%02d %02d", ti->tm_year + 1900, ti->tm_mon + 1, ti->tm_mday,ti->tm_hour);
}

void asTime::GetLocalTimeStrHMS(char* str)
{
	tm* ti = GetLocalTime();
	snprintf(str, TIME_STR_MAX_LEN, "%02d:%02d:%02d", ti->tm_hour, ti->tm_min, ti->tm_sec);
}
