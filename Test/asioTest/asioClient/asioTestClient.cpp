...
void asioTestClient::OnNewMessage(asAsioSession& session, u32 msgId, char* buf)
{
	if (buf)
	{
...
void asioTestClient::OnCloseSession(u32 sessionId)
{
	std::cout << "OnDeleteSession, session id : " << sessionId << std::endl;
}

void asioTestClient::OnAddNewSession(asAsioSession& session)
{
	std::cout << "OnAddNewSession, session id : " << session.GetID() << std::endl;
	m_serverSessionId = session.GetID();
}

u32 asioTestClient::GetServerSession()
{
	return m_serverSessionId;
}

void asioTestClient::DoTestSendData()
{
...
void asioTestClient::ParseTestProto(char* buf, u32 len)
{
	ResBaseInfo res;
	if (res.ParseFromArray(buf, len))
	{
		std::cout << "msg str : " << res.str() << std::endl;
	}
}
