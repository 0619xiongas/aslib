...
void asAsioSession::Init(u32 id, asio::ip::tcp::socket* sock, u32 recvBufSize, u32 sendBufSize, asAsioNetwork* network)
{
	m_id = id;
	m_network = network;
	if (!m_socket)
	{
		...
		m_socket = sock;
		m_socket->set_option(asio::socket_base::receive_buffer_size(recvBufSize));
		m_socket->set_option(asio::socket_base::send_buffer_size(sendBufSize));
...
asio::ip::tcp::socket* asAsioSession::GetSocket()
{
	return m_socket;
}
