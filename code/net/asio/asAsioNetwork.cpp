...
bool asAsioNetwork::TryRunNetwork(bool isClient)
{
	if(m_threads == nullptr && !m_isStoped)
	{
		...
		else
		{
			...
			try
			{
				m_acceptor = new asio::ip::tcp::acceptor(m_threads[0]->m_ioc);
...
u32 asAsioNetwork::ConnectToServer()
{
	...
	asio::error_code ec;
	asio::ip::tcp::socket* sock = new asio::ip::tcp::socket(m_threads[0]->m_ioc);
...
void asAsioNetwork::DoSendData(char* buf, u32 len, u32 sessionId)
{
	...
	if (m_threads && m_threads[threadID])
	{
		...
		m_threads[threadID]->m_ioc.post(std::bind(&asAsioNetwork::HandleSendData, this, threadID, sessionId, sbuf, len));
	}
	else
	{
...
void asAsioNetwork::DoSendDataAll(char* buf, u32 len)
{
	if (m_threads)
	{
		...
		for (u32 i = 0; i < m_threadNum; ++i)
		{
			if (m_threads[i])
			{
				m_threads[i]->m_ioc.post(std::bind(&asAsioNetwork::HandleSendDataAll, this, i, sbuf, len));
			}
		}
	}
	else
...
void asAsioNetwork::DoSendDataSome(char* buf, u32 len, std::vector<u32>& sessions)
{
	if (m_threads)
	{
		...
		for (size_t i = 0; i < sessions.size(); ++i)
		{
			...
			{
				m_threads[threadID]->m_ioc.post(std::bind(&asAsioNetwork::HandleSendData, this, threadID, sessions[i], sbuf, len));
			}
		}
	}
	else
...
{
	u32 id = ++m_sessionIDAlloc;
	asio::ip::tcp::socket* sock = new asio::ip::tcp::socket(m_threads[id % m_threadNum]->m_ioc);
	m_threads[id % m_threadNum]->m_sockTmp = sock;
	m_acceptor->async_accept(*sock, std::bind(&asAsioNetwork::HandleAccept, this, id, sock, std::placeholders::_1));
}

void asAsioNetwork::HandleAccept(u32 sessionID, asio::ip::tcp::socket* sock, const asio::error_code& ec)
{
	...
	if (sock)
	{
		if (!ec)
		{
			if (this->GetSessionCount() > m_maxSessionCount)
			{
				...
				sock->shutdown(asio::ip::tcp::socket::shutdown_both, ec1);
				sock->close(ec2);
				delete sock;
				m_threads[sessionID % m_threadNum]->m_sockTmp = nullptr;
			}
			else
			{
				...
				u32 threadID = sessionID % m_threadNum;
				m_threads[threadID]->m_ioc.post(std::bind(&asAsioNetwork::HandleAddSession, this, threadID, sessionPtr));
...
{
	sessionPtr->m_socket->async_read_some(asio::buffer(sessionPtr->m_recvBuf.CurrentBuf(), sessionPtr->m_recvBuf.RemainSize()),
		std::bind(&asAsioNetwork::HandleReceived,this,std::placeholders::_1,std::placeholders::_2,sessionPtr));
}

void asAsioNetwork::HandleReceived(const asio::error_code& ec, size_t bytestransfered, asAsioSession* sessionPtr)
{
	if (!sessionPtr)
	{
...
void asAsioNetwork::HandleCloseSession(u32 threadID, u32 sessionID)
{
	...
	if (sessionPtr)
	{
		...
		{
			asio::error_code ec1, ec2;
			sessionPtr->m_socket->shutdown(asio::ip::tcp::socket::shutdown_both, ec1);
...
{
	if (sessionPtr->m_pending) // 正在发送中，数据添加到队列中
	{
		sessionPtr->m_sendQueue.Push(buf, len);
	}
	else if (sessionPtr->m_sendQueue.m_size > 0) // 发送队列中有数据，合并队列发送
	{
		sessionPtr->m_sendQueue.Push(buf, len);
	...
	else
	{
		...
		asio::async_write(*sessionPtr->m_socket, asio::buffer(buf.get(),len),
			std::bind(&asAsioNetwork::HandleWrited,this,std::placeholders::_1,std::placeholders::_2,sessionPtr,buf));
	}
}
...
void asAsioNetwork::SendSessionQueue(asAsioThread* pThread, asAsioSession* sessionPtr)
{
	...
	asio::async_write(*sessionPtr->m_socket, asio::buffer(wb.Buf(), wb.Size()),
		std::bind(&asAsioNetwork::HandleWrited, this, std::placeholders::_1, std::placeholders::_2, sessionPtr, buf));
}

void asAsioNetwork::HandleWrited(const asio::error_code& ec, size_t bytestransfered, asAsioSession* sessionPtr, AS_SHARED_ARRAY<char> buf)
{
	u32 sessionID = sessionPtr->GetID();
	if (ec)
	{
	...
	else
	{
		if (sessionPtr)
		{
			...
			if (sessionPtr->m_sendQueue.m_size > 0) // 发送队列中还有数据，继续发送
			{
