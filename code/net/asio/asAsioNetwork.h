...
protected:
	...
	void HandleAccept(u32 sessionID, asio::ip::tcp::socket* sock, const asio::error_code& ec);

	void HandleAddSession(u32 threadID, asAsioSession* sessionPtr);

	void StartReceive(asAsioSession* sessionPtr);

	void HandleReceived(const asio::error_code& ec, size_t bytestransfered, asAsioSession* sessionPtr);
	...
	void HandleWrited(const asio::error_code& ec, size_t bytestransfered, asAsioSession* sessionPtr, AS_SHARED_ARRAY<char> buf);
